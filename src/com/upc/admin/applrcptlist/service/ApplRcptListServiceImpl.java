...
public class ApplRcptListServiceImpl implements ApplRcptListService {
	...
	public ApplRcptRstVO applRcptCmbRst() {
		...
		log.info("request : 결과취합  ");
		
		try {
			applRcptRstVO = appRcptListDAO.getApplRcptCmbRst(); 			
			
		}catch(Exception e) {
			log.error("Error occurred : 데이터베이스 오류 (getApplRcptCmbRst)");
		}
		
		log.info("response : 결과취합 (applRcptRstVO) ");
	...
	public ArrayList<ApplRcptListVO> applList(SearchVO searchVO) {
		...
		try {
			...
			log.error("Error occurred : 데이터베이스 오류 (getApplList)");
		}
		
		log.info("response : 발급신청 목록(getApplList) ");
		return appList;
	}
	...
	public int wrtrApptd(ApplRcptListVO applRcptListVO) {
		...
		log.info("request : 데이터베이스 업데이트 (담당자 지정)");
		rst = appRcptListDAO.updateWrtrApptd(applRcptListVO);
		
		if(rst != StsCode.FAIL) {
			log.info("response : SUCCESS(1) (담당자 지정)");
