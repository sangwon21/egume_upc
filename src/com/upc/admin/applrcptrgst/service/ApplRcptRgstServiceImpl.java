...
public class ApplRcptRgstServiceImpl implements ApplRcptRgstService {
	...
	public ApplRcptListVO applRcptOne(ApplRcptListVO applRcptListVO) {
		log.info("reqeust : 데이터베이스 Select (발급신청 상세)");
		applRcptListVO = applRcptRgstDAO.getApplRcptOne(applRcptListVO);
		log.info("response :  applRcptListVO (발급신청 상세)");
	...
	public ArrayList<FileVO> fileInfoList(ApplRcptListVO applRcptListVO) {
		...
		log.info("reqeust : 데이터베이스 Update (근거서류 등록, 수정)");
		if(fileList.size() != 0) {
		...
		log.info("response : 근거서류 리스트 (파일 등록, 수정)");
		return prsfileList;
	}
	...
	public void modApplRgst(ReqFrmVO reqFrmVO,FileVO fileVO, MultipartFile[] tax, MultipartFile[] sup
		...
		// 직접등록
		if(reqFrmVO.getAppmthd().equals("ADMIN") && reqFrmVO.getApplnum().equals("")) {
			
			log.info("request : 데이터베이스 insert(발급신청내역 등록)");
			...
			log.info("request : 데이터베이스 update(발급신청내역 수정)");
		...
		log.info("request : 데이터베이스 update or insert(발급신청내역 근거서류 등록)");
